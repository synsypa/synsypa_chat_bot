...
def respond(input_str):
    if len(input_str.split()) > 15:
        trunc_str = ' '.join(input_str.split()[0:15])
    else:
        trunc_str = input_str
        
    # Clean out custom emoji
    trunc_str = re.sub("([\<]).*?([\>])", "", trunc_str).strip()
    # Remove punctuation that end sentences
    trunc_str = re.sub("[\.\?\!\,](?!\w)", "", trunc_str)
    
    input_vector = get_test_input(trunc_str, corpus, max_encode)
